...
class Server extends \Home {
	...
	function Buy($f3) {
		...
		if ( ! $account->check($f3->get('POST.user'))) {
			$this->flash('Akun Ini Udah Sayang Cuba Yang Lain Dong');
			$f3->reroute($f3->get('URI'));
		}
		...
		if ($f3->exists('POST.pass',$pass)) {
			if ( ! \Check::Confirm('POST.pass')) {
				$this->flash('Ketik Yang Benar Woy !!!!!');
				$f3->reroute($f3->get('URI'));
			}
			$account->pass = $account->crypt($pass);
		}
		$active = date("Y/m/d",strtotime("+31 days"));
		$account->expire = \Webmin::exp_encode($active);
		if( ! $account->save()) {
			$this->flash('Yah Gagal Mungkin Di Suruh Cium Dulu nih ');
			$f3->reroute($f3->get('URI'));
		}
		$this->me->saldo = $this->me->saldo-$server->price;
		$this->me->save();
		$this->flash('Cie Akun Berhasil Di Buat','Akun Telah Di Buat');
